...
3. Read the printed table. One row per recorded request: host, path, status, content type. Each bookmaker's Premier League URL must show status 200; the exit code is 1 otherwise.
...
The recorder visits, per bookmaker, the site root (`_url`) and the exact Premier League URL the library builds, with the library's own headers. The HAR keeps only requests to `bet9ja.com`, `betking.com`, `nairabet.com`, and `biahosted.com` hosts, with response bodies embedded. It writes the decoded Premier League body per bookmaker as `<bookmaker>-premier-league.json` next to the HAR; a body is the source for a canned fixture under `tests/fixtures/` when a payload shape changes.

## Review the manifest entry

Copy the printed facts into `tests/fixtures/endpoint-provenance.json`:

- Top level: `recorded-at`, `egress` (`city`, `country`, `provider`, `ip`), `resolver`, `method` (`playwright-har`), `script`, `recorded-by`.
- One entry per host under `hosts`: `bookmaker`, `host`, `role` (`site`, `api`, or `referer`), `status`, `resolved-ip`, and `sample-url` for an `api` host.
- A host the package stopped calling moves to `superseded` with `status`, `checked-at`, and a `note`.
...
The recording proves what the browser saw on one date from one egress. A bookmaker can move a host without notice; the weekly live workflow catches the move as a `BookmakerUnreachableError` or a `BookmakerBlockedError` before the next release.
